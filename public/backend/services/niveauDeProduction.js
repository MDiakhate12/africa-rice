...
const getNiveauDeProductionById = async (id) => {
  const niveauDeProduction = await service.findByPk(NiveauDeProduction, id)
  if (niveauDeProduction) {
    console.log("GET ONE", niveauDeProduction.toJSON());
    return niveauDeProduction.toJSON();
  }
  console.log("GET ONE", niveauDeProduction);
  return niveauDeProduction;
}
